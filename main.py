    ...
    while trading_task_running:
        try:
            ...
            for signal in signals:
                ...
                signal['stop_loss'] = round(signal['entry_price'] - stop_loss, 5) if signal[
                                                                                         'type'] == 'LONG' else round(
                    signal['entry_price'] + stop_loss, 5)
                signal['take_profit'] = round(signal['entry_price'] + take_profit, 5) if signal[
                                                                                             'type'] == 'LONG' else round(
                    signal['entry_price'] - take_profit, 5)
                ...
                if signal['type'] == 'LONG':
                    order = await exchange.create_limit_buy_order(symbol, amount=trade_amount,
                    ...
                    order = await exchange.create_limit_sell_order(symbol, amount=trade_amount,
                                                                   price=signal['entry_price'])
...
async def trading_task():
    ...
    try:
        ...
        symbol = os.getenv('PAR')  # Perpetual Futures simbol
        with open('/app/data.json', 'r') as f:
            data = json.load(f)
        leverage = data.get('leverage', 2)
        trade_amount = data.get('trade_amount', 0.01)
